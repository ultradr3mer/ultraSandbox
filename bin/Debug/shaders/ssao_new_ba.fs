...
uniform float totStrength = 3.0;
uniform float strength = 1;
uniform float rad = 1.3;
uniform float falloff = 0.2;
uniform float totHighlightStr = 0.03;
...
void main() {
	...
	vec3 fres = normalize(vec3(texture(Texture2,dynamicPos)) * 2 - 1);
	...
	if(curpixel.a == 100){
		...
		{	
			...
			dist = 1/pow(length(ray),1.5);
			
			// calculate the cavity for highlights
			raydir = normalize(-ray * rad)*vec3(1,1,0);
			highlight += dot(raydir, sample.xyz)*fallof*dist;
			*/
		}
		
		// output the result
		float ao = (totStrength*bl+highlight*totHighlightStr)/samples;
		//ao = 0.5-highlight*totHighlightStr/samples;
	   
		out_frag_color = vec4(curpixel.xyz*0.5+0.5,0.5-ao);
	}
}